...
description: Inspect Relewise Retail Media advertisers, campaigns, Display Ads, locations, and campaign performance, or explicitly update advertiser state and campaign approval, schedule, or budget.
---
...
Use `RetailMediaGetCampaignPerformance` for a requested UTC interval of at most 366 days; future end times are capped at now. Preserve the source time buckets and entity breakdowns.

Keep campaign reporting distinct from ordinary Core Retail Media API consumption. Sponsored Product views, sales quantity, and revenue cover all channels and are not ad-attributed conversions. Keep revenue separated by currency and never total different currencies. Display Ads provide promotion and click metrics; do not rename promotions as another metric. Do not infer spend or ROAS because these are not supplied.
...
After updating, report the Dataset, entity, exact changed fields, and returned state. Do not claim excluded create/delete, location or creative editing, Display Ad Type management, Quality Score, spend, or ROAS capabilities.
