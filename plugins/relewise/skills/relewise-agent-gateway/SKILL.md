...
OAuth-capable MCP clients should use the Agent Gateway Connected App flow. For the bundled CLI, direct REST, or MCP clients without OAuth, prefer an already configured vendor-protected credential or secure credential provider. The portable fallback is `RELEWISE_AGENT_GATEWAY_TOKEN`.
