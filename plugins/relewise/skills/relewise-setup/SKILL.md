...
Before any Agent Gateway call, activate and follow the installed `relewise-agent-gateway` skill from this plugin. Pass it the identity operation or MCP tool; do not resolve the CLI, choose a transport, or handle transport mechanics in this setup skill. This setup is idempotent: verify existing authentication before changing anything, and stop as soon as a reusable route works within the user's requested authentication method. For an explicit OAuth-only requirement, use the host-managed Agent Gateway MCP connection throughout setup and verification.
