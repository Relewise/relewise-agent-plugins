...
The bundled CLI, the registered Agent Gateway MCP server, and direct Agent Gateway REST requests use the same PAT. Make it available as `RELEWISE_AGENT_GATEWAY_TOKEN` through one durable route:
...
Both routes are supported. The plugin's MCP configuration reads the variable without storing the PAT in the repository. Never place the PAT in an AI conversation, visible command text, command arguments, output, logs, URLs, or repository files. The setup must not require the user to paste or export the PAT again before each use.
