...
& "$PSScriptRoot/write-plugin-configuration.ps1" -PluginName relewise -Vendor google -OutputRoot $packageRoot -Version (Get-Content -Raw "$repositoryRoot/version.json" | ConvertFrom-Json).version
